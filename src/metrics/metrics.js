...
export class NullMetrics {
	constructor(elements) {
		this.nullObject = this.createNullObject(elements);
	}




createNullObject(elements) {
...
export class FrequencyMetrics {
	constructor(elements) {
		this.freqObject = this.createFreqObject(elements);
		this.fields = elements.map(element=> {
			return {freqMap: new Map(), freqTmp: new Map(), tmpMinVal: {key:null, value:0}};
			});
		this.maxTmp = 10;
		this.distinctCount = elements.map(element=> {return {nonUnique: 0, unique: 0,duplicate: 0, distinct: 0};});
		this.totalRecords = 0;
	}
	createFreqObject(elements) {
		return elements.map(element=>{
			return {freq: []};
		});
	}
	updateFrequencies(elements) {
		if(elements.length == this.freqObject.length) {
			for(var i=0;i<this.freqObject.length;i++) {
				this.updateFreq(elements[i],i);
			}
		}
	}

	updateFreq(field, index) {
		this.totalRecords++;
...
export class DataStatistics {
	...
	constructor(elements) {
	 	...
	 	this.frequency = new FrequencyMetrics(elements);
	}

	updateStatistics(elements) {
		this.frequency.updateFrequencies(this.convertToMasks(elements));
	}

	convertToMasks(elements) {
		return elements.map(element => {
			return {value: this.removeDiacritics(element.value).replace(this.letter,'L').replace(this.digit,'D')};
		}, this);
	}
